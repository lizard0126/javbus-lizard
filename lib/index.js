...
`, import_koishi.h.image(imageData));
      ...
      const doubleForward = (0, import_koishi.h)("message", {
        forward: true,
        children: [
          (0, import_koishi.h)("message", { userId: session.userId, nickname }, forwardMessage)
        ]
      });
      try {
...
${m.link}`).join("\n\n");
  ...
  jav.action(async ({ session }, number) => {
...
`;
  ...
  jav.subcommand("jkw <keyword:text>", "关键词搜索影片").action(async ({ session }, keyword) => {
    if (!keyword) {
      return "请提供搜索关键词！";
    }
    try {
...
番号：${movie.id}`,
  ...
  jav.subcommand("jew [type:text]", "获取最新影片").action(async ({ session }, type) => {
